...
import { NEW_RECEIVING_NAMES_DAYS, NEW_UPDATED_ON_HUB_DAYS } from './constants';

// Pre-computed enriched data (generated by web/scripts/run-enrichment.js)
...
function computeIsNew(pos: { receiving_names_from?: string; updated_on_hub?: string }): boolean {
  const receivingCutoff = new Date();
  receivingCutoff.setDate(receivingCutoff.getDate() - NEW_RECEIVING_NAMES_DAYS);

  // Primary: receiving_names_from date within threshold
  const receivingDate = parseDate(pos.receiving_names_from || '');
  if (receivingDate && receivingDate >= receivingCutoff) return true;

  // Fallback: updated_on_hub within threshold
  const updatedDate = parseDate(pos.updated_on_hub || '');
  if (updatedDate) {
    const updatedCutoff = new Date();
    updatedCutoff.setDate(updatedCutoff.getDate() - NEW_UPDATED_ON_HUB_DAYS);
    if (updatedDate >= updatedCutoff) return true;
  }
