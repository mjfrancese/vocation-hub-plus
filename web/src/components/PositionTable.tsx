...
// Columns with cycling sub-fields for Location and Date
// Clicking a column header cycles: field1 asc -> field1 desc -> field2 asc -> field2 desc -> ...
const LOCATION_CYCLE: Array<{ key: SortField; label: string }> = [
  { key: 'diocese', label: 'Diocese' },
  { key: 'state', label: 'State' },
  { key: 'city', label: 'City' },
];

const DATE_CYCLE: Array<{ key: SortField; label: string }> = [
  { key: 'date', label: 'Date Posted' },
  { key: 'updated', label: 'Last Updated' },
  { key: 'firstseen', label: 'First Seen' },
];

type ColumnDef = { key: SortField; label: string; cycle?: typeof LOCATION_CYCLE };

const COLUMNS: ColumnDef[] = [
  { key: 'name', label: 'Church' },
  { key: 'diocese', label: 'Location', cycle: LOCATION_CYCLE },
  { key: 'date', label: 'Date', cycle: DATE_CYCLE },
  { key: 'quality_score', label: 'Status' },
  { key: 'asa', label: 'ASA' },
];

function getCycleLabel(cycle: typeof LOCATION_CYCLE, currentField: SortField): string | null {
  const entry = cycle.find(c => c.key === currentField);
  return entry ? entry.label : null;
}

function isInCycle(cycle: typeof LOCATION_CYCLE, field: SortField): boolean {
  return cycle.some(c => c.key === field);
}
...
export default function PositionTable({
  ...
  function handleSort(field: SortField) {
    // Find if this field belongs to a cycle group
    const col = COLUMNS.find(c => c.key === field || (c.cycle && isInCycle(c.cycle, field)));
    ...
    if (cycle) {
      // Check if current sort is within this cycle
      const currentIdx = cycle.findIndex(c => c.key === sortField);
      if (currentIdx >= 0) {
        // Already sorting within this cycle
        if (sortDir === 'asc') {
          // asc -> desc (same sub-field)
          newField = sortField;
          newDir = 'desc';
        } else {
          // desc -> next sub-field asc (or wrap to first)
          const nextIdx = (currentIdx + 1) % cycle.length;
          newField = cycle[nextIdx].key;
          newDir = 'asc';
        }
      } else {
        // Not currently in this cycle, start at first sub-field asc
        newField = cycle[0].key;
        newDir = 'asc';
      }
    } else {
      // Simple column: toggle direction
      if (sortField === field) {
        newField = field;
        ...
        <table className="w-full table-fixed divide-y divide-gray-200">
          ...
          <thead className="bg-gray-50">
            <tr>
              ...
              {COLUMNS.map((col) => {
                const isSortedHere = col.cycle
                  ? isInCycle(col.cycle, sortField)
                  : sortField === col.key;
                const cycleLabel = col.cycle && isSortedHere
                  ? getCycleLabel(col.cycle, sortField)
                  : null;
                return (
                  <th
                    key={col.key}
                  ...
                  >
                    <span className={`flex items-center gap-1 ${col.key === 'asa' ? 'justify-center' : ''}`}>
                      {col.label}
                      {isSortedHere && cycleLabel && (
                        <span className="text-primary-600 text-[10px] normal-case tracking-normal font-normal">
                          {cycleLabel}
                        </span>
                      )}
                      {isSortedHere && (
