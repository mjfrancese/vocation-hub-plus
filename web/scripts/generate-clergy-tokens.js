/**
 * Generate deterministic tokens for all clergy and precompute personal data.
 *
 * Usage:
 *   CLERGY_TOKEN_SECRET=<secret> node scripts/generate-clergy-tokens.js
 *
 * Outputs:
 *   public/data/clergy-tokens.json       (token -> PersonalData map)
 ...
 * Uses HMAC-SHA256 with CLERGY_TOKEN_SECRET env var, then base64url-encodes
 ...
 * @param {string} guid
 * @returns {object} PersonalData-shaped object
 */
function buildPersonalData(guid) {
  const db = getDb();
  ...
  // Strip "Diocese of " prefix to match the stored diocese name
  ...
  // ASA bucket median (compensation_by_asa uses median+gender columns)
  ...
  if (currentParishId) {
    ...
    if (parish?.nid) {
      ...
      if (latestParochial?.average_attendance) {
        ...
        // Try to find ASA bucket -- asa_category stored as range like "1-50", "51-100" etc.
        // Fall back gracefully if no data
        try {
  ...
  if (currentParishId) {
    ...
    if (p) {
      ...
      try {
        const censusPath = path.resolve(__dirname, '../public/data/census-data.json');
 ...
 * @returns {Array<ClaimSearchEntry>}
 */
function buildSearchIndex() {
  ...
  return allClergy.map(c => {
    ...
    const token = generateToken(c.guid);
...
/**
 * Generate all tokens and write output files.
 */
function generateAllTokens() {
  const db = getDb();
  const allClergy = db.prepare(`SELECT guid FROM clergy`).all();
  console.log(`Generating tokens for ${allClergy.length} clergy...`);
  ...
  fs.writeFileSync(path.join(DATA_DIR, 'clergy-tokens.json'), JSON.stringify(tokenMap));
  console.log(`Wrote clergy-tokens.json (${count} entries)`);

  // Write search index
  const searchIndex = buildSearchIndex();
  fs.writeFileSync(path.join(DATA_DIR, 'clergy-search-index.json'), JSON.stringify(searchIndex));
  console.log(`Wrote clergy-search-index.json (${searchIndex.length} entries)`);

  // Insert tokens into DB for reference
  const upsert = db.prepare(`INSERT OR REPLACE INTO clergy_tokens (token, clergy_guid) VALUES (?, ?)`);
  const insertMany = db.transaction((entries) => {
    for (const [token, data] of entries) {
      upsert.run(token, data.clergy_guid);
    }
  });
  insertMany(Object.entries(tokenMap));
  console.log(`Inserted ${count} tokens into clergy_tokens table`);
}
...
module.exports = { generateToken, buildPersonalData, buildSearchIndex, generateAllTokens };
